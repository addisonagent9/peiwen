...
- **task.md cleanup: 曆 pingshui gap** `88a81a4`. Added 曆 (U+66C6, traditional form, "calendar" / 曆法) to `src/data/pingshui.json` reading 入聲 十二錫, matching the existing 歷 (U+6B77) and 历 (U+5386) entries (which are present in pingshui as `[{tone:"入", group:"入聲", rhyme:"十二錫"}]`). Surfaced during #7 implementation; deferred because the analyzer's variant fallback chain (Unihan + simp/trad) handled the typed 曆 → 歷 path transparently in most analyzer surfaces. Closing as a completeness fix: direct lookup now lands without falling through to variant resolution. **Path A** (CSV-side fix): inserted `十二錫,曆` row into `~/pw/pingshui_入聲.csv` immediately after the existing `十二錫,歷` row to keep related glyphs adjacent (build-pingshui.mjs preserves CSV row order via `chars.push(word)` — no sort). Then `npm run data` regenerated `src/data/pingshui.json` and `npm run build` regenerated dist. Post-rebuild verification: 曆's entry is byte-identical to 歷's (`[{"tone":"入","group":"入聲","rhyme":"十二錫"}]`); 曆 appears in `pingshui.rhymes["十二錫"].chars[]`; codepoint U+66C6 confirmed (not 暦/U+66A6, not 歷/U+6B77). Bundle hash rotated `a29269b9 → 6c254bef` (3.71 MB). No UI / analyzer logic change; no behavioral change observable to users. Single-char data delta. CSV remains uncommitted per project convention (staging only).
- **task.md cleanup: meta.json phase merge** `(this commit)`. Patched the meta-write block in `scripts/build-unique-char-content.mjs` (~line 605-660) to merge re-run stats into existing phase blocks rather than overwrite. Surfaced during #17 Part 2 follow-up — re-running 九佳 + 十二文 for 1 char each erased B1's actual 68/93-char phase stats; worked around manually for that commit. **Patch** adds a `mergePhase(prev, curr)` function (inlined just above the existing `existingMeta` read site, no new module / dep) with explicit per-field semantics — no generic deep-merge. Field classifications: **max** for `totalGapCount` (preserves largest universe ever surveyed for this phase — protects the motivating bug case where a 1-char re-run otherwise shrinks the historical universe-size record); **additive** for `extracted`, `byConfidence.{HIGH,MEDIUM,LOW}`, `bySource.{zdic,wiktionary,llm-grounded,llm-only-audit}`, `llmCallCount`, `llmTokensIn`, `llmTokensOut`, `llmCostUSD` (parsed via `parseFloat(s.replace("$",""))` — `parseFloat` on `"$"`-prefixed strings returns NaN otherwise — re-formatted to `$X.XXXX`), `wallTimeMinutes`; **latest-wins** for `phase` (identity), `ranAt`, `throttleMs`, `model`. Missing prev fields fall through to curr's value defensively (older phase blocks predating any newer field won't crash; `prev.X ?? 0` for numerics, `prev.X ?? curr.X` for strings). Helper `addCounts(a,b)` sums nested-object sub-fields (handles `byConfidence` and `bySource` uniformly). Helper `parseCostUSD(s)` handles undefined / null / non-numeric input → 0. No `union` semantic implemented — current schema has zero array fields; defer until one exists. Merge only fires when `existingMeta.phases[PHASE]` exists; new phases still write-new (existing behavior unchanged). **Verification** via Test B synthetic harness (no API spend, deterministic): Test 1 (motivating bug case — full B1 九佳 prev + 1-char re-run curr): expected output `{totalGapCount:69 (max), extracted:69 (68+1), HIGH:62 (61+1), llmCostUSD:"$0.0053" (0.0047+0.0006), ranAt:latest, throttleMs:750 (latest), model:latest}` — PASS. Test 2 (defensive missing-prev-fields): older phase block missing newer fields merges cleanly without NaN/undefined leakage — PASS. Test 3 (parseCostUSD edge cases — undefined/null/`"$"` bare): all coerce to 0 contribution — PASS. `data/audit/unique-char-content-meta.json` is **unchanged** by this commit; existing on-disk state preserved. Offline script only — no runtime / VPS / bundle path affected.
- **Wenyan pairing — 2-char modern hints (compact mode)** `(this commit)`. New preview-and-swap feature for the wenyan pairing exercise (#26 stage C/D follow-up). The drill's right column previously showed full ancientMeaning glosses (e.g. 林栖者 → 山林隐士). New "简洁模式" toggle swaps to 2-char literary-modern pairings (e.g. 林栖者 → 隐者) during the drill phase; end-of-game results panel always reveals full ancientMeaning regardless of toggle state (pedagogical contract: drill on hints, learn on full glosses). 125 hints generated across the full corpus via Haiku (5 batches, user-reviewed and corrected per-batch). **Architecture**: hints staged in sidecar `src/data/wenyan/pairing-hints.json` (NOT merged into poems.json yet — that's a future "swap-to-canonical" follow-up). Server `/pairing/queue` response extended with `sense_slug` per meaning (2-line SQL change to extend SELECT + response builder; no migration, no new route, no DB write — `sense_slug` column already exists). Client `useWenyanApi.fetchPairingQueue()` augments response with `pairingHint` via module-level `Map<senseSlug, hint>` built once from static-imported sidecar. `WenyanPairingSession.tsx` gains `compactMode` state (default off → existing byte-identical behavior) + pill toggle button as sibling to "剩 N 对" status line + `meaningCardText(m)` accessor that respects compactMode for drill render. Existing `meaningTextFor(entryId)` accessor untouched — results-reveal panel continues to use it (always shows full ancientMeaning). **Hint generation pipeline**: new `scripts/wenyan-batch-hints.mjs` (~340 lines) — 5-batch flow with per-batch STOP gate. Haiku `claude-haiku-4-5-20251001`, no web_search (pure text), prompt evolved across batches via user-confirmed reference points. Direction pivoted mid-pipeline from "文言-register hints" to "literary-modern pairings" after Batch 1 — second-pass register preference settled on Batch 2's slightly-literary modern Mandarin (逸士 / 几何 / 蓬门 / 至诚 / 黎明 family). Proper-noun rule added in Batch 3 (typonyms / 典故 / place names → adjacent classical descriptor, not bare same-word echo; e.g. 龙城 → 王庭, 阴山 → 北岭, 飞将 → 将领). **Quality gates**: 1-char validator rejects (1 firing — 教 → 使 rejected, manually set to 任凭 with classical-context disambiguation note). Per-batch user review with CSV-form corrections protocol (parentheticals descriptive only, no-reply = accept LLM/AI suggestion). Cost ~$0.07 total across all 6 generation passes (Batch 1 fired twice due to direction pivot). **Final sidecar state**: 125/125 entries, 125 unique hints (zero collisions after 4-echo resolution: 山人 → 隐士, 前川 → 前河, 异乡 → 他乡, 唯见 → 只见), all `corrected_by: "user"`, all `confidence: "high"`. **i18n**: `compactModeLabel` + `compactModeAria` added to cn / tw / en blocks of `src/i18n/wenyan-strings.ts`. **Bundle**: `pairing-hints.json` ships via Parcel static-import inlining into JS bundle; also copied to `dist/pairing-hints.json` via `scripts/inline-bundle.mjs` for parity with dictionary convention. Build hash rotated `6c254bef → 2887c49b` (~25 KB delta from sidecar inlining). **Out of scope** (future swap-to-canonical ticket): delete sidecar, merge `pairingHint` field into `src/data/wenyan/poems.json`, remove compactMode toggle (becomes always-on), drop `compactModeLabel` / `compactModeAria` from wenyan-strings.ts. That ticket fires after some real-world drill iterations validate the hint set.
