...
export function Grid({ chars, lineTemplates, onPick, onRhymeClick }: Props) {
  return (
    <div className="flex flex-row-reverse gap-3 justify-center py-4" dir="ltr">
      {chars.map((row, li) => (
        <div key={li} className="flex flex-col gap-2 items-center">
          <div className="text-xs text-creamDim font-sans">第{li + 1}句{lineTemplates[li].rhymes ? "（韻）" : ""}</div>
          {row.map((c, i) => (
            <CharCell
              key={i}
              c={c}
